...
import {P2PEarthquake, UserCoordinates} from "./interface"



export async function getRecentEarthquake(userCoordinates : UserCoordinates, maxDistanceKM = 500){

    const response = await axios.get<P2PEarthquake[]>('LATERWILLBEADDED',
                                                      {
                                                        params: {
                                                        codes: "551",
                                                        limit:100
    ...
    const nearbyEarthquakes = earthquakes.filter(eq => {
        if(!eq.earthquake?.hypocenter ) return false;
